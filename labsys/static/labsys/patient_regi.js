...
(function () {
  ...
  document.querySelector("#fname").addEventListener("keyup", (e) => {
    if (
      ...
      document.querySelector("#lname").value.length < 3
    ) {
      e.target.focus();
      document.querySelector("#srchbtn").disabled = true;
      return;
  ...
  document.querySelector("#lname").addEventListener("keyup", (e) => {
    if (
      e.target.value.length < 3 &&
      document.querySelector("#fname").value.length < 3
    ) {
      e.target.focus();
      document.querySelector("#srchbtn").disabled = true;
      return;
      ...
      .then((names) => {
        names.forEach((name) => {
          ...
          tr.innerHTML = `<td class="view-message text-left">${name.patient_id}</td>
                        <td class="view-message text-center "><a href= '/lab/regi_${register}/${name.patient_id}'>${name.fname} ${name.lname} </a> </td>
